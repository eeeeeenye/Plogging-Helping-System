...
const LocationTracker = () => {
  ...
  const startLocationTracking = async () => {
    try {
      ...
      if (!locationSubscription) {                              // 구독이 존재하는 경우 만들지 않음 (중복방지)
        const newSubscription = Location.watchPositionAsync(    // watchPosition은 비동기 함수이고, 반환값은 _h,_i,_j(remove함수 포함 -> 제어함수들 포함)
          {
  ...
  return (
    <View style={styles.container}>
      <View style={{ position:'absolute', top: 40, left: 25, right: 300, bottom: 610, zIndex:1 }}>
