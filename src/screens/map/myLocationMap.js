...
const MylocationMap = ({ navigation }) => {
  ...
  const getLocation = async () => {
    try {
      ...
      latitude = locationData['coords']['latitude'] // 위도 가져오기
      longitude = locationData['coords']['longitude'] // 경도 가져오기
      ...
      if (data.documents && data.documents.length > 0) {
        ...
        // setPosition(firstDocument.address.address_name)
      } else {
    ...
    <View style={{ flex: 1 }}>
      <WebView
        ...
        onMessage={(event) => {
          ...
          latitude = event.nativeEvent.data.lat
          longitude = event.nativeEvent.data.lng
        }}
      />
      <View style={{ flex: 0.4 }}>
        {/* <Text>{position}</Text> */}
